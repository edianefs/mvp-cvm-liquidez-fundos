...
# DBTITLE 3,3. Preparar arquivos da CVM
...
for month in MONTHS:
    ...
    if not os.path.exists(local_zip):
        raise FileNotFoundError(
            ...
            f"Faça o upload de {zip_name} para {BASE_VOLUME}."
        )
    ...
    with zipfile.ZipFile(local_zip, "r") as zf:
        ...
        if not csv_members:
            raise ValueError(
                f"Nenhum CSV encontrado dentro de {local_zip}"
            )
    ...
    print(
        f"{zip_name}: {len(csv_members)} CSV(s) "
        f"disponível(is) em {month_dir}"
    )
...
for month in MONTHS:
    ...
    if not csv_files:
        ...
        raise FileNotFoundError(
            ...
            f"Verifique se o ZIP foi extraído corretamente."
        )
...
print("Arquivos CSV que serão lidos pelo Spark:")
...
# No Unity Catalog, input_file_name() não é utilizado.
# O caminho do arquivo é obtido pelo metadado _metadata.file_path.
...
silver_base = (
    ...
    .withColumn(
        "ID_SUBCLASSE",
        F.col("ID_SUBCLASSE").cast("string")
    )
    ...
    .withColumn(
        "VL_TOTAL",
        F.col("VL_TOTAL").cast("double")
    )

    .withColumn(
        "VL_QUOTA",
        F.col("VL_QUOTA").cast("double")
    )
    ...
    .withColumn(
        "CAPTC_DIA",
        F.col("CAPTC_DIA").cast("double")
    )

    .withColumn(
        "RESG_DIA",
        F.col("RESG_DIA").cast("double")
    )

    .withColumn(
        "NR_COTST",
        F.col("NR_COTST").cast("long")
    )
)
...
with_d1 = (
    ...
    .withColumn(
        ...
        F.when(
            F.col("vl_patrim_liq_d1") > 0,

            F.col("RESG_DIA")
            /
            F.col("vl_patrim_liq_d1")
        )
    )
...
if p95 is None:
    ...
    raise ValueError(
        "Não foi possível calcular o percentil 95."
    )
...
gold_summary = (
    ...
    .agg(
        ...
        F.min("DT_COMPTC").alias(
            "data_inicial"
        ),

        F.max("DT_COMPTC").alias(
            "data_final"
        ),

        F.avg("VL_PATRIM_LIQ").alias(
            "pl_medio"
        ),

        F.sum("CAPTC_DIA").alias(
            "captacoes_periodo"
        ),

        F.sum("RESG_DIA").alias(
            "resgates_periodo"
        ),

        F.sum("fluxo_liquido").alias(
            "fluxo_liquido_periodo"
        ),

        F.sum(
            F.when(
                F.col("fluxo_liquido") < 0,
                1
            ).otherwise(0)
        ).alias(
            "dias_fluxo_negativo"
    ...
    .withColumn(
        ...
        F.when(
            ...
            F.col("dias_validos") > 0,

            F.col("dias_fluxo_negativo")
            /
            F.col("dias_validos")
        )
    )
